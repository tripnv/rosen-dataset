...
def original_text_cleaning(text):
    # Remove PAUZĂ
    text = re.sub("PAUZĂ", '', text)
...
def get_timestamp_for_steno_2(steno, df_transcript):
    ...
    df_transcript['Count'] = df_transcript.groupby(g)['in_corpus'].transform('size') * np.where(
        df_transcript['in_corpus'], 1, -1)
...
def assign_timestamps_to_steno(steno_fpath, transcript_fpath):
    ...
    try:
        ...
        df_steno['original_text'] = df_steno['text']
        df_steno['original_text'] = df_steno['original_text'].map(original_text_cleaning)
